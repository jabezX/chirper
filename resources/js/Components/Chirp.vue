...
<template>
    <div class="px-6 py-4 flex space-x-2">
        <svg
            ...
            class="h-4 w-4 mt-1 text-gray-600 -scale-x-100"
            fill="none"
            viewBox="0 0 25 25"
            stroke="currentColor"
            stroke-width="2"
        ...
        <div class="flex-1">
            ...
            >
                <textarea
                    ...
                    class="mt-4 w-full text-gray-900 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm"
                ></textarea>
                <InputError :message="form.errors.message" class="mt-2" />
                <div class="space-x-2">
                    <PrimaryButton class="mt-4 text-sm">Save</PrimaryButton>
                    <button
                        class="mt-4 text-sm"
                        @click="
                            editing = false;
            ...
            <p v-else class="text-sm text-gray-600">{{ chirp.message }}</p>
        </div>
    </div>
</template>
